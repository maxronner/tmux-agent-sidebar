...
- **tmux window** — the slug is used as the window name; the agent you picked in the modal (`claude` / `codex` / `opencode` / `pi`) launches inside it with the selected permission `MODE`.
...
Worktree lifecycle tracking is Claude-only today — it needs the `WorktreeCreate` / `WorktreeRemove` hooks, which Codex, OpenCode, and Pi do not emit.
